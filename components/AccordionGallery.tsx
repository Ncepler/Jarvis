...
// Vendored from React Bits (reactbits.dev) — AccordionGallery, JS+CSS variant
// converted to TypeScript to match this repo's convention (no .jsx/.js files
// anywhere in components/). Logic is kept as close to the original as
// possible; the only addition is the optional `onActiveIndexChange` callback
// below, needed so a caller can show a persistent CTA that tracks whichever
// panel is currently expanded (this repo's Gallery section needs that — the
// original component has no way to observe its internal `active` state).
...
export type AccordionGalleryProps = {
  ...
  // Second addition (see file header): `target` for every panel's `<a>`,
  // e.g. "_blank" so following a panel's link never navigates the caller
  // away from the gallery itself.
      ...
      {items.map((item, i) => {
        ...
        return (
          <Tag
            ...
            target={item.link ? linkTarget : undefined}
            rel={item.link && linkTarget === "_blank" ? "noopener noreferrer" : undefined}
            onClick={(e: React.MouseEvent) => handleClick(i, e)}
            onMouseEnter={() => handleEnter(i)}
            onFocus={() => setActive(i)}
            onKeyDown={(e: ReactKeyboardEvent) => handleKeyDown(i, e)}
            role="listitem"
            tabIndex={0}
