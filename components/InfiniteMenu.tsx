...
// Vendored from React Bits (reactbits.dev) — InfiniteMenu, JS+CSS variant
// converted to TypeScript to match this repo's convention (no .jsx/.js files
// anywhere in components/). Logic and rendering are kept as close to the
// original as possible; two deliberate additions:
//
// 1. `#initTexture`'s image loader now resolves `null` on a failed load
//    (instead of never resolving) so one bad/CORS-blocked screenshot can't
//    hang the whole atlas forever — failed items just render an empty cell.
// 2. Each image is drawn into its square atlas cell with a proper cover-crop
//    (scale to fill, crop the overflow, top-aligned) instead of a naive
//    stretch-to-fit `drawImage`. This repo's client-site screenshots are
//    16:10 landscape captures, not square, and stretching them to a square
//    cell would visibly distort every logo/hero shot; cropping matches the
//    `object-cover object-top` convention already used everywhere else this
//    exact screenshot type renders (components/sections/ClientSites.tsx).
...
class InfiniteGridMenu {
  ...
  constructor(
    ...
    this.canvas = canvas;
    this.items = items || [];
    this.onActiveItemChange = onActiveItemChange || (() => undefined);
    this.onMovementChange = onMovementChange || (() => undefined);
  ...
  private initTexture() {
    ...
    Promise.all(
      this.items.map(
        (item) =>
          new Promise<HTMLImageElement | null>((resolve) => {
            ...
            // A bad/CORS-blocked screenshot resolves to null rather than
            // hanging Promise.all forever — that cell just stays blank.
      ...
      images.forEach((img, i) => {
        ...
        // Cover-crop into the square cell (scale to fill, crop the
        // overflow, anchored to the top) instead of stretching — see the
        // file header comment.
...
export default function InfiniteMenu({ items = [], scale = 1.0, backgroundColor = "#000000" }: InfiniteMenuProps) {
