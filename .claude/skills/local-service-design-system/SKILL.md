...
- **Renovation / remodeling:** Block Renovation, Bill Huey + Associates, CASE Architects & Remodelers, Normandy Remodeling, Harvest Homes. Patterns: minimal and **gallery-driven** ("let the work speak"), refined whitespace, **before/after room sliders**, a **portfolio organized by room** (kitchen / bath / whole-home), and a **process / step-by-step narrative** (design → build) with scope + expected timeline. *(See §14e.)*
...
5. ⟨adapts⟩ **Services section ("what we do")** — **every niche now has a tailored layout in §14** (the niche layout is the default; build that). The numbered 01–0N card grid (number marker, H3 title, 1–2 sentence description, "Explore →", optional 4:3 thumbnail) is a building block some layouts still use — e.g. it's the base of renovation's service list — but don't drop the same grid on every site.
...
8. ⟨adapts⟩ **Value props / "Why hire us"** — **see the niche's tailored layout in §14.** Where a niche doesn't specify otherwise, use numbered items 01–0N (big number, H3 title, 1–2 sentence body). Several niches replace this with a niche-specific treatment (proof strip, spec strip, narrative band, "the chair").
...
|---|---|---|---|---|
| **Renovation / remodeling** *(BUILD FIRST, near-black §2, layout §14e)* | `#C8893F` warm finished-wood caramel | Kitchens ● Bathrooms ● Additions ● Basements ● Whole-Home ● Trim & Carpentry | 01 Kitchens · 02 Bathrooms · 03 Additions & extensions · 04 Basements · 05 Whole-home renovations · 06 Trim & finish carpentry | "Old house. / New everything." |
| **Landscaping** (Stone & Sage, forest-dark §13g, layout §14f) | `#6E9A5C` moss/sage green | Patios ● Retaining Walls ● Gardens ● Lighting ● Fire Pits | 01 Design · 02 Patios & walkways · 03 Retaining walls · 04 Garden & planting · 05 Maintenance · 06 Custom (fire pits, pergolas) | "Built to be lived in. / Built to stay." |
...
**Every niche has a tailored layout below.** The numbered 01–0N card grid (§6.5/§6.8) is still a useful building block — renovation and landscaping use it for their service *list* — but it is no longer the default treatment for any niche on its own. Build the niche layout. The point: a barber's "services" is a price board, a lawn-care "services" is a plan selector, a florist's is occasion tiles. Match the section to the business.
...
### 14e. Renovation — before/after rooms + process (Maple & Main)
Renovation's proof is the *transformation*, and homeowners' biggest fear is the unknown of the process. Lead with both. Mood: near-black §2, minimal and gallery-driven — the photography does the talking.
- **Services section ("What we do")** — keep the numbered room list (01 Kitchens · 02 Baths · 03 Additions · 04 Basements · 05 Whole-home · 06 Trim), but make the section's centerpiece a **room before/after slider** (a kitchen gut, a bath) — draggable `clip-path` reveal, accent handle. Lead with one or two big room before/afters, then the service list beneath. The cards can stay as a compact list since the sliders carry the visual weight.
- **Add a "How it goes" process timeline** — a horizontal numbered stepper, **Design → Demolition → Build → Finish**, with a connecting hairline, a one-line "what happens" under each, and an honest duration note ("most kitchens: 4–6 weeks"). This reassures more than any copy. (Renovation is the niche that most needs the process made visible.)
- **Work grid → filterable by room** — chips that filter the grid: Kitchen / Bath / Addition / Whole-home / Exterior. Same tile component, just filterable.
- **Value-props ("Why hire us")** — keep the numbered 01–05 list (one crew every trade · built to last · straight written pricing · local & responsive · clean and on schedule); renovation is the base, so the numbered list is fine here.
- *Build note:* before/after sliders reuse the §14a/§14d slider component. Placeholders `BEFORE — kitchen (16:9)` / `AFTER — kitchen (16:9)` until real photos.

### 14f. Landscaping — day↔night lighting + filtered portfolio (Stone & Sage)
The signature move: show a yard **by day and by night with the lighting on** — exactly how landscape designers sell lighting ("a rendering by day, one by night; that's when people see the magic"). Mood: forest-dark §13g.
- **Signature interactive — day↔night toggle.** A featured outdoor space (patio / pool / garden) with a **toggle or slider that switches it from daylight to dusk with the landscape lighting on** — warm uplighting, path lights, fire feature glowing. This is landscaping's "moving part," and it demonstrates the lighting service better than any list. Build as a full-bleed break or a dedicated "After dark" section. Implementation: cross-fade two images (day / night) on toggle, or a before/after-style slider between them. Reduced-motion/mobile: a simple two-button day|night cross-fade, or the two images stacked.
- **Services section ("What we do")** — keep the numbered service list (01 Design · 02 Patios & walkways · 03 Retaining walls · 04 Garden & planting · 05 Maintenance · 06 Custom), but the day↔night feature above is the section's hero; the list sits beneath it.
- **Work grid → filterable by type** — chips: Patios / Walls / Gardens / Lighting / Fire pits. Each tile a real project with a one-line caption.
- **Add an "Our process" stepper** — **Design → Build → Grow** (consult/3D plan → install/hardscape → planting & maintenance), a short horizontal stepper like renovation's.
- **Value-props ("Why hire us")** — keep the numbered list (designed in-house · built to last · honest pricing · local · we maintain what we plant).
- *Build note:* placeholders `FEATURED — patio, DAY (16:9)` / `FEATURED — patio, NIGHT, lights on (16:9)` for the toggle; the night image is where Noah's future Higgsfield/real dusk shots drop in.

### 14g. Lawn care — service plans + instant estimate (Fresh Cut)
Lawn care is a **recurring-plan** business, and the whole funnel is *get a quote*. So the "services" section is a plan selector, not a project grid. Mood: light fresh §13e.
- **Services section ("What we do") → service-plan tier cards.** Replace the numbered project grid with **3 plan cards** — e.g. **Basic Mow** (weekly cut, edge, blow), **Full Care** (mow + fertilizing + weed control), **Seasonal** (cleanups + mulch + everything) — each card lists what's included and a **"from $X / visit"** (clearly a starting price). Lay them out like pricing tiers; mark one "most popular." This reads as a plan you subscribe to, which is what lawn care is.
- **Add an instant-estimate widget — "get a price in 10 seconds."** Pick **lawn size** (Small / Medium / Large, or a sq-ft slider) + **frequency** (weekly / biweekly) → a **price estimate updates live** (label it *"rough estimate, not a quote"*). The estimate CTA pre-fills the contact form. This is lawn care's distinctive moving part (the TruGreen quote-builder pattern, simplified).
- **Value-props ("Why us")** — a compact strip: same crew each week · text before we come · easy to cancel · we show up. Plain, friendly.
- **Work grid** — keep it, but lawn care's gallery is striped-lawn / before-after-cleanup shots; a simple grid is fine.
- *Build note:* the estimate is just a small lookup table (size × frequency → range); no backend. Keep numbers obviously illustrative on the demo.

### 14h. Florist — occasion browsing + bouquet gallery (Wildstem)
People shop a florist **by occasion**, not by service number, and the arrangements are the whole show. Mood: light/airy serif §13a — neutral background, photography does the work.
- **Services section ("What we do") → occasion tiles.** Replace the numbered cards with **4 large photographic occasion tiles** — **Weddings · Sympathy · Everyday · Events** — each a beautiful full-bleed-ish image with the occasion name in the display serif, hover/tap lifting it slightly. People self-select by why they're buying. (These map to the §9 services: weddings/events, daily, sympathy, weekly.)
- **Add "the shop" — a bouquet gallery.** A grid of arrangements with **big photography and thin white gutters** (the bouquet-menu pattern), each with a short name + optional "from $X." Lets flowers carry the page. This can double as the work grid or sit alongside it.
- **Optional — weekly flowers.** A small **subscription block** (fresh flowers weekly / biweekly) — a real florist revenue line and an easy, calm CTA.
- **Value-props ("Why us")** — fold into a short warm line or keep a light 3-item set (locally grown where we can · arranged the day it ships · real florist, not a wire service). Soft, not corporate.
- *Build note:* serif headers (Fraunces/Playfair) used with restraint; soft shadows, no hard borders; placeholders `OCCASION — Weddings (3:4)`, `BOUQUET — seasonal (4:5)`.
