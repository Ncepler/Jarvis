...
- Done (committed, NOT verified on live deploy or phone yet):
  ...
  - **Gallery rework (2026-06-11, Noah request, two rounds):** click a card → the site's REAL homepage opens full-bleed below the row, physically part of the page (Noah rejected anything that reads as a "tab inside the site"). Preferred path: homepage as a React component in `components/demos/`, registered by slug in `components/demos/index.ts` — rendered inline (no iframe) in the panel, as the card's scaled live thumbnail, and as the section backdrop. First demo: lawn-care ("Fresh Cut Lawn Co." sample brand, slug `demo-lawncare`, isStyleDemo:true → contact form style select is back). Third-party placeholders use mirrored HTML in `public/previews/<slug>.html` (`node scripts/mirror.mjs <slug> <url>`) shown in a seamless AUTO-HEIGHT same-origin iframe (content measured, no inner scrollbar, sandboxed). Fallbacks: live iframe (desktop, embeddable:true, fixed window) → screenshotFull. Add-a-site steps at top of `lib/projects.ts`. CLAUDE.md §6.3/§7 updated to match.
...
1. Verify ALL features on live + phone-width + reduced-motion in a real browser — nothing is verified beyond builds + curl. Especially: (a) the lawn-care demo inline (panel, card thumbnail, backdrop), (b) mirrored previews auto-height — confirm nike/apple/allbirds/terminal/relats render AND get measured to full height (their JS runs against our origin — some may degrade or report bad heights)
