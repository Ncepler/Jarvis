...
- **Gallery elevation** (`components/sections/Gallery.tsx`): cards now carry `category` + `caption` (from `lib/projects.ts`, Axel "Recent work" anatomy) and are live `DemoScaled` mini-previews. Hovering the centered card lifts + sharpens it (spring scale + saturate/contrast filter, on an inner element so it doesn't fight the layout box). A trailing **"step inside →" cursor** (`StepInsideCursor`, spring inertia) follows the pointer over the row; hover-only (`md:cursor-none` on the row), never on touch/reduced.
  - **Open interaction = `layoutId` morph (brief #1, supersedes §6.3's always-on panel, Noah 2026-06-19):** clicking the centered card (or Enter / tap) morphs that card's media box — shared `layoutId={`demo-<slug>`}` — into the full live interactive demo panel below. One open at a time; `Close ✕` / Esc collapses it and returns focus to the row; dragging the row closes the open demo. It still lives **in the page flow** (no floating window, keeps §6.3's "scroll straight through it" rule) — the morph just replaces the auto-show. Reduced motion drops the `layoutId` (plain fade open/close); the flat scroll-row's taps open the same panel. Caveat to verify in-browser: the demo panel is full homepage height, so the FLIP animates a tall box — looked correct in SSR/build but needs Noah's eyes for feel.
...
- **Homepage panel:** clicking the active card opens a full-bleed panel under the row and scrolls it into view. Inside is the project's REAL homepage, physically part of the page — the visitor scrolls our page and flows straight through it. It must never feel like a tab/window inside the site (Noah, 2026-06-11).
- **Inline demos (preferred — our own builds):** the homepage lives as a React component in `components/demos/`, registered by slug in `components/demos/index.ts`. The gallery renders the real component inline — no iframe — and also uses it scaled-down as the card's live thumbnail and the section backdrop. Demos are self-contained (own colors, sample brand clearly demo-labeled, no real contact info).
- **Mirrored previews (third-party placeholders):** homepage HTML in `public/previews/<slug>.html` (`preview` field), captured via `node scripts/mirror.mjs <slug> <url>` or pasted by hand with a `<base href>` tag. Served from our origin so X-Frame-Options can't block it; shown in a seamless auto-height iframe (content height measured, no inner scrollbar), sandboxed without `allow-top-navigation`. Lazy-mount only on open.
- **Fallbacks:** no demo and no `preview` → live iframe of `url` (desktop only, requires `embeddable: true`, fixed-height window) → `screenshotFull` capture.
